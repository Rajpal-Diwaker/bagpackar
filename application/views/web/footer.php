                  ...
                  <div class="lsf_wrapper forgotwr">
                     <div class="lsf_formfield">
                        ...
                        <form id="forgot_form" method="post">
                           ...
                           <div class="lst_footer">
                              <div class="form-group">
                                 <input type="submit" value="Forgot" class="btn_style" id="forgot_submit">
                              </form>
      ...
      if(google_id!=""){
         ...
         $.ajax({
          ...
          success: function(response){ 
              ...
              if(response=='block'){
               $(".loaderCntr").hide();  
               alertify.notify('Your account has been blocked by the admin, Please contact support for further assistance.', 'error', 4);
               return false; 
            }
               //============user deleted==============//
               else if(response=='deleted'){
                  $(".loaderCntr").hide();  
                  alertify.notify('Invalid credentials.', 'error', 4);
                  return false; 
               }
               //=============user login suceesfully========================//
               else if(response !=0 && response!="new"){
                 ...
                 if(page==1){
                  ...
                  if(page==3 && $('#check_validation').val()=='1'){      
                    payment();
                 }
              }
              $(".loaderCntr").hide();
...
function send_otp1(phone){
   ...
   $("#forgot_submit").on('click',function(event){
      var formStatus=true;
      var forgot_email = $("#forgot_email").val();
   ...
   if (formStatus==false) { 
      ...
      $.ajax({
         ...
         success: function(response){
          ...
          $(".loaderCntr").hide();
          alertify.notify("Incase of social login, you can't change the password from Bagpackar",'error', 5)
          return false; 
       }
       else {
          $(".loaderCntr").hide();
          alertify.notify("Email doesn't exist!",'error', 4)
          return false; 
       } 
    }
 });
...
<script type="text/javascript">
   var cookieConsent = new cookieConsent({
      clientId: 'e50d6e8e-c8f7-4a51-9dfd-497f52e7765e' 
   });
   cookieConsent.run();
</script>