  ...
  <div class="dashboard_data_wrap">
    <div class="dahaboard_heading">
      <span>Edit Most Popular Image</span>
    </div>
    <!-- User Details Section Start Here-->
         ...
         <form method="post" enctype="multipart/form-data" id="most_popular_form">
          <div class="col-sm-9 col-lg-6 col-md-8 col-xs-12">
           ...
           <div class="user_profile_wrap clearfix ">
            <label for="">Most Popular Image</label>  
            <div class="edit_profile_img_wrapper">
              <div class="edit_profile_img">
                <img id="profileimg" src="<?php echo $data['image'];?>" alt="">
              </div>
              <div class="edit_profile_upload-icon-img">
                <i id="profileuploadicon" class="fa fa-camera" aria-hidden="true"></i>
              </div>
              <input id="profile_img_Upload" type="file" class="hidden-lg hidden-md hidden-sm hidden-xs" placeholder="Photo" capture="" value="<?php echo $data['image'];?>">
              <span style="margin-top: 2px;color:red;font-weight: bold;" id="image_error"></span>

            </div>
          </div>
          <input type="hidden" name="latitude" id="latitude" value="<?php echo $data['latitude'];?>">
          <input type="hidden" name="longitude" id="longitude" value="<?php echo $data['longitude'];?>">
          <div class="user_profile_wrap clearfix ">
            <label for="">Address</label>
            <input class="form-control" type="text" name="address" id="address" placeholder="Please enter address" value="<?php echo $data['address'];?>">
            <span class="errors_form" id="address_error"></span>
          </div>
          <div class="user_profile_wrap clearfix ">
            <label for="">Country</label>
            <input class="form-control" type="text" id="country" name="country" placeholder="Please enter country" value="<?php echo $data['country'];?>">
            <span class="errors_form" id="country_error"></span>
          </div>
          <div class="user_profile_wrap clearfix ">
            <label for="">City</label>
            <input class="form-control" type="text" id="city" name="city" placeholder="Please enter city" value="<?php echo $data['city'];?>">
            <span class="errors_form" id="city_error"></span>
          </div> 
          <div class="user_profile_wrap clearfix">
            <label for="">Title</label>
            <input class="form-control" type="text" id="title" name="title" placeholder="Please enter title" value="<?php echo $data['title'];?>">
            <span class="errors_form" id="title_error"></span>
          </div> 
          <div class="user_profile_wrap clearfix ">
            <label for="">Body</label>
            <textarea id="body" name="body" rows="10" cols="110"><?php echo $data['body'];?></textarea>
...
$('#body').on('keyup',function(){
  ...
  remain = limit - parseInt(len);
  $('#body').text(remain);
  $("#body_error").html('The maximum 500 character are allowed').show().fadeOut(3000);
}
...
{ 
  ...
  $.ajax({
    url: "<?php echo base_url()?>admin/Admin/edit_popular_process",
    type: 'POST',
    data:data,
    success: function(response){
      if(response ==1){
        alertify.notify('Edit Successfully', 'success', 4); 
        setTimeout(function() { 
