...
<div class="dashboard_data_wrap">
  ...
  <div class="dashbord_cont_wrapper">
    <div class="user_details_sec clearfix">
      <div class="row">
       <form method="post" enctype="multipart/form-data" id="discover_form">
        <div class="col-sm-9 col-lg-6 col-md-8 col-xs-12">
          ...
          <div class="user_profile_wrap clearfix ">
            <label for=""><?php echo $image_label;?></label>  
            <div class="edit_profile_img_wrapper">
              <div class="edit_profile_img">
                <img id="profileimg" src="<?php echo $result['image'];?>" alt="">
              </div>
              <div class="edit_profile_upload-icon-img">
                <i id="profileuploadicon" class="fa fa-camera" aria-hidden="true"></i>
              </div>
              <input id="profile_img_Upload" type="file" class="hidden-lg hidden-md hidden-sm hidden-xs" placeholder="Photo" capture="" value="<?php echo $result['image'];?>">
              <span style="margin-top: 2px;color:red;font-weight: bold;" id="image_error"></span>

            </div>
          </div>
          <input type="hidden" name="latitude" id="latitude" value="<?php echo $result['latitude'];?>">
          <input type="hidden" name="longitude" id="longitude" value="<?php echo $result['longitude'];?>">

          <div class="user_profile_wrap clearfix">
            <label for="">Address</label>
            <input class="form-control" type="text" name="address" id="address" placeholder="Please enter address" value="<?php echo $result['address'];?>">
            <span class="errors_form" id="address_error"></span>
          </div>
          <div class="user_profile_wrap clearfix ">
            <label for="">Country</label>
            <input class="form-control" type="text" id="country" name="country" placeholder="Please enter country" value="<?php echo $result['country'];?>">
            <span class="errors_form" id="country_error"></span>
          </div>
          <div class="user_profile_wrap clearfix ">
            <label for="">City</label>
            <input class="form-control" type="text" id="city" name="city" placeholder="Please enter city" value="<?php echo $result['city'];?>">
...
$("#discover_form").on('submit',function(event){
  var formStatus=true;
  var address = $("#address").val();
  var country = $("#country").val();
  var city = $("#city").val();
...
{ 
  ...
  $(".loaderCntr").show();
  var data=[];
  var data=$("#discover_form").serializeArray();
  $.ajax({
    url: "<?php echo base_url()?>admin/Admin/edit_image_process",
    type: 'POST',
    data:data,
    success: function(response){
      if(response ==1){
        var notification = alertify.notify('Updated Successfully', 'success', 4, function(){ 
         console.log('dismissed'); 
       })
        setTimeout(function() { 
          window.location.href = '<?php echo base_url().$redirect_path;?>';
        }, 500); 
      }
      if(response==0){
       $(".loaderCntr").hide();
