  ...
  <div class="dashboard_data_wrap">
    ...
    <div class="dashbord_cont_wrapper">
      <div class="user_details_sec clearfix">
        <div class="row">
         <form method="post" enctype="multipart/form-data" id="popular_form">
...
$("#popular_form").on('submit',function(event){
  var formStatus=true;
  var address = $("#address").val();
  var country = $("#country").val();
  var city = $("#city").val();
...
{ 
  ...
  $(".loaderCntr").show();
  var data=[];
  var data=$("#popular_form").serializeArray();
  $.ajax({
    url: "<?php echo base_url()?>admin/Admin/popular_destination_process",
    type: 'POST',
    data:data,
    success: function(response){
      if(response ==1){
        ...
        setTimeout(function() { 
          window.location.href = '<?php echo base_url();?>admin/popular-destination-list';
        }, 500); 
      }
      if(response==0){
       $(".loaderCntr").hide();
